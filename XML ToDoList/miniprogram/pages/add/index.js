...
Page({
  ...
  data: {
    ...
    time:`${Date.now()}-${Math.floor(Math.random(0, 1) * 1000)}`,
  },
  ...
  addFile: function () {
    ...
    wx.chooseImage({
      ...
      res => {
        wx.showLoading({//showloading 加载动画
          title: '上传中',
        })

        setTimeout(function () {//showloading 动画消失
          wx.hideLoading()
        },1000)//时间

      //console.log(res)
      
      console.log(this.data.time)  // this.data.title 为 name path(id) title
      const file = res.tempFiles[0]
      // 上传文件至云存储
      getApp().uploadFile(this.data.time+res.tempFilePaths[0].match(/\.[^.]+?$/)[0],file.path).then(res => {
        // 追加文件记录，保存其文件名、大小和文件 id
        this.data.files.push({
          name: this.data.time,
          //size: (file.size / 1024 / 1024).toFixed(2),
        ...
        this.setData({
          files: this.data.files,
          fileName: this.data.fileName + this.data.time + ' '
        })
      })
    })
  },
